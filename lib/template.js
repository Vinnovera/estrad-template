	...
	module.exports = function (opts) {
		...
		this.solveDependencies = function(obj, callback) {
			...
			while(Object.keys(this.dependees).length) {
				...
				if(this.caughtInLoop(this.dependees)) break;
		...
		this.evaluateDependency = function(key) {
			...
			if(this.dependencies.hasOwnProperty(key) &&
				...
				for(i = 0, len = this.dependees[key].length; i < len; i++) {
					...
					this.garbageShield(content, this.dependees[key][i], key);
				}
		...
		this.garbageShield = function(content, dependee, key) {
			// part
			this.content[dependee] = interpolate(this.content[dependee], content);

			// Remove solved dependencies
			this.dependencies[dependee].splice(this.dependencies[dependee].indexOf(this.dependees[key]), 1);
		...
		this.caughtInLoop = function(dependees) {
			var tmp = JSON.stringify(dependees);
