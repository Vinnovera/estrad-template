(function(){
	...
	module.exports = function (opts) {
		...
		this.evaluateDependency = function(key) {
			...
			if(this.dependencies.hasOwnProperty(key) &&
				...
				for(i = 0, len = this.dependees[key].length; i < len; i++) {
					...
					this.garbageShield(content, this.dependees[key][i]);
				}
		...
		this.garbageShield = function(content, dependee) {
			// part
			...
			if(!this.dependencies[dependee].length) {
				...
				this.content[dependee] = mData.template(this.content[dependee], this.data[dependee]);
			}
		};
